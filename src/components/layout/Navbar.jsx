...
import { categories } from "@/data/catSubcatMap";
          ...
          <div className="flex flex-col gap-1">
            {categories.map((category) => (
              <div key={category.name}>
                <button
                  className="flex w-full items-center justify-between px-3 py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-foreground/10 rounded"
                  onClick={() =>
                    setOpenDropdown(openDropdown === category.name ? false : category.name)
                  }
                >
                  {category.name}
                  <ChevronDown
